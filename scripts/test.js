...
function getHistoryRef() {
  if (typeof firebase === "undefined" || !firebase.apps || !firebase.apps.length) {
    throw new Error("Firebase app is not initialized. Check scripts/testfirebase.js");
  }
...
function createScrGroupElement(outputList, airportCode, scrOutput) {
  ...
  showScrBtn.classList.add("secondary");
  ...
  copyScrBtn.classList.add("secondary");
  ...
  copyEmailBtn.classList.add("secondary");
...
function formatChangeSCRMessages() {
  ...
  Object.values(airportGroups).forEach((grp, gIdx) => {
    ...
    fCPs.forEach((pair) => {
      ...
      cLT.style.cssText = "font-family: monospace; white-space: pre; font-size: 11px; color: #9fb2ca;";
...
function renderHistoryChangeEditor(entry, itemContainer) {
  ...
  viewHighlightedBtn.classList.add("secondary");
...
function renderHistoryPanel() {
  ...
  filtered.forEach(({ entry, index: realIndex }) => {
    ...
    inputBtn.classList.add("secondary");
    ...
    removeBtn.style.boxShadow = "0 8px 20px rgba(239,83,80,0.18)";
