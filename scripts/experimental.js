...
const airportInfoCache = new Map();
...
async function getAirportOperationalInfo(airportCode) {
  const code = String(airportCode || "").trim().toUpperCase();
  if (!code) return null;

  if (airportInfoCache.has(code)) {
    return airportInfoCache.get(code);
  }
  ...
  const lookupPromise = (async () => {
    ...
    try {
      const snapshot = await firebase.database().ref("airports/" + code).once("value");
      return snapshot.exists() ? snapshot.val() : null;
    } catch (error) {
      console.error(`Airport information lookup failed for ${code}:`, error);
      return null;
    }
  })();

  airportInfoCache.set(code, lookupPromise);
  return lookupPromise;
}

function formatAirportLevel(value) {
  const text = String(value ?? "").trim().toUpperCase();
  return text || "N/A";
}

function getAirportLevelClass(value) {
  const level = formatAirportLevel(value);
...
function formatAirportRequirement(value) {
  ...
  if (["YES", "Y", "TRUE", "1", "REQUIRED"].includes(text)) return "YES";
  if (["NO", "N", "FALSE", "0", "NOT REQUIRED"].includes(text)) return "NO";

  return text;
}

function getAirportRequirementClass(value) {
  const status = formatAirportRequirement(value);
  if (status === "YES") return "requirement-yes";
  if (status === "NO") return "requirement-no";
  return "requirement-na";
}

function createAirportSummaryElement(airportCode, slotLabel = "") {
  const code = String(airportCode || "").trim().toUpperCase();

  const summary = document.createElement("div");
  summary.className = "airport-summary";

  const heading = document.createElement("div");
  heading.className = "heading airport-result-heading";

  const title = document.createElement("span");
  title.className = "airport-result-title";
  title.textContent = code;

  const badge = document.createElement("span");
  badge.className = "airport-slot-badge";
  badge.textContent = slotLabel || "SCR";
  ...
  infoBar.className = "airport-operational-info airport-info-loading";
  infoBar.textContent = "Loading airport information...";
  summary.appendChild(infoBar);

  getAirportOperationalInfo(code).then((airportData) => {
    if (!airportData) {
      title.textContent = code;
      infoBar.className = "airport-operational-info airport-info-unavailable";
      infoBar.textContent = "Airport information unavailable";
      return;
    }

    const icao = String(airportData.airportIcao || "").trim().toUpperCase();
    const airportName = String(airportData.airportName || "").trim();

    const titleParts = [code];
    if (icao) titleParts[0] += ` / ${icao}`;
    if (airportName) titleParts.push(airportName);
    title.textContent = titleParts.join(" — ");

    const summerLevel = formatAirportLevel(airportData.summerLevel);
    const winterLevel = formatAirportLevel(airportData.winterLevel);
    const gcr = formatAirportRequirement(airportData.gcr);
    const ppr = formatAirportRequirement(airportData.ppr);
    const currentSeason = getIATASlotSeasonUTC().startsWith("S") ? "summer" : "winter";

    infoBar.className = "airport-operational-info";
    infoBar.innerHTML = "";
    ...
    const items = [
      {
        label: "Summer Level",
        value: summerLevel,
        valueClass: `airport-level ${getAirportLevelClass(airportData.summerLevel)}`,
        current: currentSeason === "summer"
      },
      {
        label: "Winter Level",
        value: winterLevel,
        valueClass: `airport-level ${getAirportLevelClass(airportData.winterLevel)}`,
        current: currentSeason === "winter"
      },
      {
        label: "GCR",
        value: gcr,
        valueClass: `requirement-status ${getAirportRequirementClass(airportData.gcr)}`,
        current: false
      },
      {
        label: "PPR",
        value: ppr,
        valueClass: `requirement-status ${getAirportRequirementClass(airportData.ppr)}`,
        current: false
      }
    ];
    ...
    items.forEach((item) => {
      ...
      itemEl.className = "airport-info-item" + (item.current ? " current-season" : "");

      const labelEl = document.createElement("span");
      labelEl.className = "airport-info-label";
      labelEl.textContent = item.label;

      if (item.current) {
        const currentEl = document.createElement("span");
        currentEl.className = "season-current";
        currentEl.textContent = "CURRENT";
        labelEl.appendChild(currentEl);
      }

      const valueEl = document.createElement("span");
      valueEl.className = item.valueClass;
      valueEl.textContent = item.value;

      itemEl.appendChild(labelEl);
      itemEl.appendChild(valueEl);
      infoBar.appendChild(itemEl);
    });
  });
...
function createEditableRLineRow(rowData) {
  ...
  const timeField = createField("Time", "text", "r-time", rowData.time);
  timeField.input.placeholder = "HH:MM";
  timeField.input.maxLength = 5;
  timeField.input.inputMode = "numeric";
  timeField.input.pattern = "^([01]\\d|2[0-3]):([0-5]\\d)$";
  ...
  function detectAircraftTypeFromCombinedCode(code) {
    ...
    if (upper.includes("7MJ")) return "7MJ";
  ...
  function recalculateCombinedCodeFromService() {
    ...
    if (selectedService === "J") {
      ...
      else if (detectedAircraft === "320") newCode = "180320";
      else if (detectedAircraft === "7MJ") newCode = "2287MJ";
      else newCode = "18973H";
    } else if (selectedService === "P") {
      if (detectedAircraft === "7M8") newCode = "0007M8";
      else if (detectedAircraft === "320") newCode = "000320";
      else if (detectedAircraft === "7MJ") newCode = "0007MJ";
      else newCode = "00073H";
  ...
  function normalizeTimeInput(value) {
    ...
    if (/^\d{1,2}:\d{1,2}$/.test(v)) {
      const [hh, mm] = v.split(":");
      v = `${hh.padStart(2, "0")}:${mm.padStart(2, "0")}`;
    }
  ...
  function validateTimeField() {
    const normalized = normalizeTimeInput(timeField.input.value);
    timeField.input.value = normalized;

    const valid = /^([01]\d|2[0-3]):([0-5]\d)$/.test(normalized);
    timeField.input.style.borderColor = valid || !normalized ? "" : "#cc0000";
    return valid;
  }
  ...
  timeField.input.addEventListener("blur", () => {
    validateTimeField();
    updateRowChangeHighlight(rowDiv);
  });

  timeField.input.addEventListener("input", () => {
    updateRowChangeHighlight(rowDiv);
  });
...
function createScrGroupElement(outputList, airportCode, scrOutput) {
  ...
  scrGroup.appendChild(
    createAirportSummaryElement(airportCode, currentSlotType || "SCR")
  );
...
function formatChangeSCRMessages() {
  ...
  Object.values(airportGroups).forEach((grp, gIdx) => {
    ...
    scrGroup.appendChild(
      createAirportSummaryElement(airportCode, "CHANGE SCR")
    );
...
async function buildUpdatedChangeScrFromContainer(containerDiv, originalPairs, headerLines, airportCode, serviceType) {
  ...
  modifiedRows.forEach((rowDiv) => {
    ...
    try {
      ...
      if (!fV || !sD || !dayV || !cCV || !/^([01]\d|2[0-3]):([0-5]\d)$/.test(tVR) || !aV || !sV) {
...
// =========================================================
// MULTIFLIGHT PAGE-SPECIFIC SCRIPT
// Moved from multiflight.html
// =========================================================
(() => {
      "use strict";
      ...
      const aircraftByScrCode = new Map();
      const aircraftRecordsByAirline = new Map();

      let aircraftDatabaseReadyPromise = null;
      let aircraftDatabaseReadyResolve = null;
      let aircraftDatabaseReadyReject = null;
      let aircraftDatabaseInitialised = false;

      let pendingAircraftContext = null;
      let aircraftModalResolver = null;
      let generationInProgress = false;

      let liveAircraftCheckTimer = null;
      let liveAircraftCheckRunning = false;
      let lastCancelledAircraftCheck = "";
      ...
      function aircraftKey(iataCode, aircraftCode) {
        return (
          cleanCode(iataCode) +
          "::" +
          cleanCode(aircraftCode)
        );
      }

      function buildAircraftRecord(
        airline,
        databaseCode,
        data
      ) {
      ...
      function cacheAircraftRecord(record, databaseCode = "") {
        if (
          !record ||
          !record.iataCode ||
          !record.inputCode
        ...
        if (
          !aircraftRecordsByAirline.has(
            record.iataCode
          )
        ) {
          aircraftRecordsByAirline.set(
            record.iataCode,
            []
          );
        }
      ...
      function rebuildAircraftCache(snapshot) {
        airlineByIata.clear();
        aircraftByCode.clear();
        aircraftByScrCode.clear();
        aircraftRecordsByAirline.clear();
      ...
      function startAircraftDatabaseListener() {
        aircraftDatabaseReadyPromise =
          new Promise(
            (resolve, reject) => {
              aircraftDatabaseReadyResolve =
                resolve;

              aircraftDatabaseReadyReject =
                reject;
            }
          );
        ...
        firebase
          ...
          .on(
            ...
            error => {
              ...
              showError(
                "Aircraft Type Database could not be loaded."
              );
            }
          );
      }

      function getAircraftFromCache(
        iataCode,
        aircraftCode
      ) {
        const key =
      ...
      async function findAircraftType(
        iataCode,
        aircraftCode
      ) {
        await aircraftDatabaseReadyPromise;
        ...
        const directSnapshot =
          await firebase
            .database()
            .ref(
              `aircraftTypes/${airline.key}/aircraft/${cleanAircraft}`
            )
            .once("value");
      ...
      function parseAircraftToken(value) {
        const token =
          String(
            value || ""
          );
      ...
      function getSelectedDAircraftCode() {
        return document
          .getElementById(
            "bombardierCheckbox"
          )
        ...
        function(input) {
          ...
          lines.forEach(
            line => {
              ...
              const aircraftToken =
                `${airlineIata}::${originalAircraftType}`;
              ...
              const flight =
                `${prefix}${digits.padStart(3, "0")}${suffix}`;
      ...
      window.getFinalCombinedCode =
        function(
          ...
          if (
            ...
            aircraftCode =
              getSelectedDAircraftCode();
          }

          if (!iataCode) {
            throw new Error(
              "Unable to determine airline for aircraft lookup."
            );
          }
          ...
          if (!aircraft) {
            throw new Error(
              `Aircraft ${aircraftCode || "UNKNOWN"} is not configured for airline ${iataCode}.`
            );
          }
      ...
      function findAircraftFromCombinedCode(
        iataCode,
        combinedCode
      ) {
        const combined =
      ...
      window.createEditableRLineRow =
        function(rowData) {
          ...
          const combinedCodeField =
            createField(
              "Code(Prefix+Op)",
              "text",
              "r-combined-code",
              rowData.combinedCode
            );
          ...
          function recalculateCode() {
            ...
            if (!aircraft) {
              showError(
                `Unable to identify aircraft for ${rowData.flight} from Firebase.`
              );
            ...
            if (
              typeof updateRowChangeHighlight ===
              "function"
            ) {
              updateRowChangeHighlight(
                rowDiv
              );
            }
          }
          ...
          timeField.input
            .addEventListener(
              ...
              () => {
                ...
                timeField.input.style.borderColor =
                  valid ||
                  !value
                    ? ""
                    : "#cc0000";
          ...
          if (
            typeof attachRowChangeTracking ===
            "function"
          ) {
            attachRowChangeTracking(
              rowDiv
            );
          }
      ...
      function updateRegistrationFromInput() {
        const registrations =
          new Set();
        ...
        if (
          ...
          regInput.classList.add(
            "reg-detected"
          );

          regInput.dataset.autoDetected =
            "true";
          ...
          regInput.classList.remove(
            "reg-detected"
          );
        }
      }

      function setAircraftModalStatus(
        message,
        type = "info"
      ) {
        const status =
      ...
      function openAircraftModal(
        ...
        document.getElementById(
          "unknownAircraftAirline"
        ).textContent =
          `${airline.airlineName} (${airline.iataCode} / ${airline.icaoCode})`;
      ...
      function requestAircraftDefinition(
        airline,
        aircraftCode
      ) {
        if (
      ...
      async function saveNewAircraftType() {
        if (
          !pendingAircraftContext
        ) {
          return;
        ...
        if (
          ...
          setAircraftModalStatus(
            "Aircraft input code is invalid.",
            "error"
          );
        ...
        if (
          ...
          setAircraftModalStatus(
            "Total seats must be between 0 and 999.",
            "error"
          );
        ...
        try {
          const ref =
            firebase
              .database()
              .ref(
                `aircraftTypes/${context.airlineKey}/aircraft/${inputCode}`
              );
          ...
          if (
            ...
            setAircraftModalStatus(
              "Aircraft already exists. The existing record will be used.",
              "success"
            );

            lastCancelledAircraftCheck =
              "";
          ...
          setAircraftModalStatus(
            "Saving aircraft type to Firebase...",
            "info"
          );
          ...
          setAircraftModalStatus(
            `${inputCode} added to ${context.airlineName}.`,
            "success"
          );
          ...
          setAircraftModalStatus(
            "Unable to save aircraft type. Check Firebase rules.",
            "error"
          );
      ...
      async function checkAircraftInputAutomatically() {
        if (
          liveAircraftCheckRunning ||
          pendingAircraftContext
        ) {
          return;
        }
        ...
        try {
          ...
          for (
            ...
            if (
              !airlineIata ||
              airlineIata.length !== 2 ||
              !aircraftCode ||
              aircraftCode.length < 2
            ) {
              continue;
            }
            ...
            const added =
              await requestAircraftDefinition(
                result.airline,
                aircraftCode
              );
          ...
          console.error(
            "Automatic aircraft check failed:",
            error
          );
      ...
      function scheduleAutomaticAircraftCheck() {
        if (
          liveAircraftCheckTimer
        ) {
          clearTimeout(
            liveAircraftCheckTimer
          );
        ...
        liveAircraftCheckTimer =
          setTimeout(
            () => {
              checkAircraftInputAutomatically();
            },
            600
          );
      }

      async function verifyAircraftBeforeGeneration() {
        const serviceType =
          cleanCode(
            document
              .getElementById(
        ...
        for (
          ...
          if (
            ...
            showError(
              `Airline ${airlineIata} is not available in the Aircraft Type Database.`
            );
          ...
          if (!added) {
            showError(
              `SCR generation cancelled because aircraft ${aircraftCode} is not configured for ${result.airline.airlineName}.`
            );
          ...
          if (
            ...
            showError(
              `Aircraft ${aircraftCode} could not be loaded after saving.`
            );
      ...
      document.addEventListener(
        ...
        () => {
          ...
          userInput.addEventListener(
            "input",
            () => {
              updateRegistrationFromInput();

              lastCancelledAircraftCheck =
                "";

              scheduleAutomaticAircraftCheck();
            }
          );
            ...
            .addEventListener(
              ...
              () => {
                if (
                  ...
                  lastCancelledAircraftCheck =
                    aircraftKey(
                      pendingAircraftContext.iataCode,
                      pendingAircraftContext.aircraftCode
                    );
                }
            ...
            .addEventListener(
              ...
              async event => {
                ...
                try {
                  ...
                  const valid =
                    await verifyAircraftBeforeGeneration();
                  ...
                  console.error(
                    "SCR generation error:",
                    error
                  );

                  showError(
                    error.message ||
                    "SCR generation failed."
                  );
