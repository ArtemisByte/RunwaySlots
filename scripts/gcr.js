(function () {
  ...
  async function incrementStat(mode, amount = 1) {
    if (!db) return;
    const add = Math.max(1, Number(amount) || 1);
    try {
      await Promise.all([
        db.ref(`${GCR_STATS_PATH}/${mode}`).transaction((current) => (Number(current) || 0) + add),
        db.ref(`${GCR_STATS_PATH}/TOTAL`).transaction((current) => (Number(current) || 0) + add)
      ]);
    } catch (err) {
  ...
  function createHistoryRecord({ mode, apt, rawInput, text, emails, airportInfo, entries, generationId }) {
    return {
      generationId,
      mode,
      airport: apt,
  ...
  /*
   * Save every airport generated by a single click in ONE atomic Firebase update.
   * This prevents a partial save where (for example) LIME is written but LEMG is not.
   */
  async function saveHistoryBatch(items, mode, rawInput) {
    if (!db || !items.length) return new Map();
    ...
    items.forEach((item) => {
      const key = db.ref(GCR_HISTORY_PATH).push().key;
      if (!key) throw new Error("Unable to create Firebase history key.");
      ...
      const record = createHistoryRecord({
        ...
        text: item.text,
        emails: item.emails,
        airportInfo: item.airportInfo,
        entries: item.relevantEntries,
        generationId
      });

      updates[`${GCR_HISTORY_PATH}/${key}`] = record;
      updates[`${GCR_BACKUP_PATH}/${key}`] = record;
      keyMap.set(item.apt, key);
    });

    // Atomic: either the complete multi-airport history batch is accepted or none is.
    await db.ref().update(updates);

    // Stats are intentionally separate from the history write.
    incrementStat(mode, items.length);
  ...
  function renderHistory() {
    ...
    const filtered = historyRecords.filter((item) => {
      const itemMode = String(item.mode || "").toUpperCase();
      const modeOk = modeFilter === "ALL" || itemMode === modeFilter;
  ...
  function setupHistoryListener() {
    ...
    const historyRef = db.ref(GCR_HISTORY_PATH);

    // No limitToLast(), no query limit: this reads every visible GCR history child.
    historyRef.on("value", (snap) => {
      ...
      console.info(`[GCR History] Loaded ${historyRecords.length} visible record(s) from /${GCR_HISTORY_PATH}`);
      renderHistory();
    }, (err) => {
  ...
  async function generateGcr() {
    ...
    try {
      ...
      // Build every airport block first.
      const preparedBlocks = airports.map((apt) => {
        ...
        return {
          ...
          relevantEntries: entries.filter((e) => e.from === apt || e.to === apt)
        };
      });

      setLoading(true, 65, `Saving ${preparedBlocks.length} GCR history record(s)...`);

      let historyKeys = new Map();
      let historySaved = false;

      if (db) {
        try {
          historyKeys = await saveHistoryBatch(preparedBlocks, mode, rawInput);
          historySaved = true;
          console.info(`[GCR History] Saved complete batch of ${preparedBlocks.length} record(s).`);
        } catch (err) {
          console.error("Atomic GCR history save failed:", err);
          showError(
            `GCR messages were generated, but the shared history batch could not be saved: ${err?.message || err}`
          );
        }
      }
      ...
      preparedBlocks.forEach((item) => {
        renderOutputBlock({
          apt: item.apt,
          mode,
          lines: item.result.lines,
          editors: item.result.editors,
          emails: item.emails,
          airportInfo: item.airportInfo,
          historyKey: historyKeys.get(item.apt) || null
        });

        generated += 1;
        const pct = 70 + Math.round((generated / preparedBlocks.length) * 30);
        setLoading(true, pct, `Rendering GCR ${generated}/${preparedBlocks.length}`);
      });

      const dbSuffix = db ? "" : " Database offline: history could not be saved.";
      const fallbackSuffix = fallbackCount
        ? ` ${fallbackCount} airport(s) were not found in the airport database.`
        : "";
      const historySuffix = db
        ? (historySaved
            ? ` ${preparedBlocks.length} shared-history record(s) saved.`
            : " Shared history save failed; check the browser console/Firebase rules.")
        : "";

      showSuccess(
        `Generated ${generated} GCR block(s) in ${displayModeLabel(mode)} mode.${historySuffix}${fallbackSuffix}${dbSuffix}`
      );
    } catch (err) {
      console.error(err);
